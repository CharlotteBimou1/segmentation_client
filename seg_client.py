...
st.write('Échantillon de données en grappes(groupe) montrant la récurrence, la fréquence et les attributs monétaires de chaque client')
...
st.write('Ensemble de données transnationales qui contient toutes les transactions effectuées entre le 01/12/2010 et le 09/12/2011 pour un commerce de détail en ligne non-magasin basé au Royaume-Uni et enregistré. De nombreux clients de cette entreprise sont des grossistes.')
...
def main():
    ...
    col2.subheader('Fréquence versus Récence')
    fig = px.scatter(
        df_pd,
        x="Frequency",
        y="RECENCY",
    ...
    col2.subheader('Fréquence versus Monétaire')
    fig = px.scatter(
        df_pd,
        x="Frequency",
        y="Monetary",
    ...
    col2.subheader('Récense versus Monétaire')
    fig = px.scatter(
        df_pd,
        x="RECENCY",
        y="Monetary",
